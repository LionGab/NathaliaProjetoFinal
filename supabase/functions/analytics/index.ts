...
async function handleTrackEvents(
  ...
  console.log(
    `[ANALYTICS] Tracked ${validEvents.length} events for ${userIdHash}`
  );
  ...
  try {
    ...
    switch (path) {
      ...
      case "stats": {
        ...
        return jsonResponse(stats, 200, allowOrigin);
      }
      ...
      case "session": {
        ...
        return jsonResponse({ session_id: sessionId }, 200, allowOrigin);
      }
