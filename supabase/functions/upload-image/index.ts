      ...
      if (data.length >= 30) {
        ...
        while (i < data.length - 10) {
          ...
          if (chunkId === "VP8 ") {
            ...
            if (signature === 0x2f) {
              const bits = data[i + 9] | (data[i + 10] << 8) | (data[i + 11] << 16) | (data[i + 12] << 24);
              const width = (bits & 0x3fff) + 1;
              const height = ((bits >> 14) & 0x3fff) + 1;
              return { width, height };
            }
          }
          const chunkSize = data[i + 4] | (data[i + 5] << 8) | (data[i + 6] << 16) | (data[i + 7] << 24);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!image) {
      return jsonResponse({ error: "Missing required field: image" }, 400, allowOrigin);
    }
    ...
    if (!decoded) {
      return jsonResponse(
        { error: "Invalid image format or unsupported type" },
        400,
        allowOrigin
      );
    }
    ...
    console.log(`[UPLOAD-IMAGE] Success: ${hashUserId(userId)} -> ${storagePath} (${imageData.length} bytes)`);
    ...
    return jsonResponse(response, 200, allowOrigin);
  } catch (error) {
