...
export async function generateImage(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      logger.error("Image generation API error response", "AssetGenerationService", undefined, { errorData, status: response.status });
      throw new Error(`Image generation API error: ${response.status} ${JSON.stringify(errorData)}`);
    }

    const result = await response.json();
    logger.info("Image generated successfully", "AssetGenerationService", { imageUrl: result.data?.imageUrl });

    // Return the image data from the response
    if (result.success && result.data) {
      return result.data.imageUrl as string;
    } else {
      logger.error("Invalid response format from API", "AssetGenerationService", undefined, { result });
      throw new Error("Invalid response format from API");
    }
  } catch (error) {
    logger.error("Image generation error", "AssetGenerationService", error instanceof Error ? error : new Error(String(error)));
    throw error;
  }
}
