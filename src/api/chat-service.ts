...
export const getOpenAITextResponse = async (messages: AIMessage[], options?: AIRequestOptions): Promise<AIResponse> => {
  try {
    ...
    logger.error("OpenAI API error", "ChatService", error instanceof Error ? error : new Error(String(error)));
    throw error;
  }
};
...
export const getGrokTextResponse = async (messages: AIMessage[], options?: AIRequestOptions): Promise<AIResponse> => {
  try {
    ...
    logger.error("Grok API error", "ChatService", error instanceof Error ? error : new Error(String(error)));
    throw error;
  }
};
