...
import { useCheckInStore, useHabitsStore } from "@/state/store";
...
function generateMoodInsight(
  ...
  if (trend === "down" && avgMood < 3.5 && daysAnalyzed >= 5) {
    return {
      ...
      message: "Notei uma tendência de queda no seu bem-estar. Vamos conversar sobre o que está acontecendo?",
      emoji: "🌧️",
      actions: {
        primary: {
          label: "Conversar com NathIA",
...
function generateEnergyInsight(avgEnergy: number, avgSleep: number, daysAnalyzed: number): HealthInsight | null {
  // Energia muito baixa
...
function generateHabitsInsight(
  ...
  if (completionRate < 0.25 && avgMood < 3) {
    return {
      ...
      message: "Notei que você não tem cuidado muito de si. Um pequeno gesto já faz diferença. Que tal começar com algo simples?",
      emoji: "🌸",
      actions: {
        primary: {
...
function generatePositiveInsight(
  ...
  if (avgMood >= 4 && avgEnergy >= 4 && completionRate >= 0.5) {
    return {
      ...
      message: "Seu humor, energia e autocuidado estão ótimos. Continue assim, você merece se sentir bem!",
      emoji: "✨",
      actions: {
        primary: {
...
export function useHealthInsights(): UseHealthInsightsReturn {
  ...
  const insights = useMemo(() => {
    ...
    const hasHighPriority = generatedInsights.some((i) => i.priority === "high" || i.priority === "medium");
